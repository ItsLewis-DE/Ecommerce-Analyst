...
purchases_schema = DataFrameSchema(
    {
        ...
        "purchase_id": Column(str, Check.str_matches(r'^[0-9a-fA-F\-]+$'), nullable=False),
        ...
        "order_date": Column(str, nullable=True),
    },
    strict=False, 
    coerce=True   
)   
    ...
    if file_name == 'purchases':
        try:
            ...
            for chunk_df in dfs_iterator:
                ...
                chunk_df['order_date'] = pd.to_datetime(chunk_df['order_date'], format="%Y-%m-%d %H:%M:%S")
